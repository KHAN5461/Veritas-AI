...
export default function PwaDebugPage() {
  ...
  const forceRegister = async () => {
    ...
    try {
      ...
      await reg.update();
      log(`Registered v8! scope=${reg.scope}, state=${(reg.active || reg.waiting || reg.installing)?.state}`);
      ...
      {loading ? <p>Checking status...</p> : swInfo && (
        <>
          <div style={{ marginBottom: '20px' }}>
            <h2 style={{ fontSize: '16px', marginBottom: '8px', color: '#aaa' }}>Service Worker (v8)</h2>
